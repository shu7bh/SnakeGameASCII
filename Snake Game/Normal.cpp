...
{
	this->food.insert	// inserting the normalFood in a map requires a key
	(
		std::pair<Coordinates, Food>
		(
			Coordinates(r, c),
			Food(r, c, types[val].getPoints(), types[val].getShape())
		)
	);
}
...
bool Types::destroyIfEaten(const unsigned int& x, const unsigned int& y, bool& addAtEnd)
{
	...
	for (it = food.begin(); it != food.end(); it++)
		if (it->first.getX() == x && it->first.getY() == y)
		{
			...
			addAtEnd = it->second.changeLength();
